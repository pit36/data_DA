...
for num, algo in enumerate(dataDict.keys()):
    # max_steps = np.mean([max(dataDict[algo][it]["step"]) for it in dataDict[algo].keys()])
    # print(algo,max_steps)
    latencies = np.array(
    ...
    for n, latency in enumerate(latencies):
        ...
        ax4.plot(range(len(mv_avg)), mv_avg)
...
fig4.savefig("mov_avg.pdf")
fig.savefig('steps.pdf')
# fig2.tight_layout(rect=[0, -0.02, 1.02, 0.99])
fig2.tight_layout()
fig2.savefig(args.save + "/nsfnet_jsac.pdf")
plt.show()
